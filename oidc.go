...
func NewProvider(ctx context.Context, issuer string, validIssuers ...string) (*Provider, error) {
	...
	if !isIssuerValid {
		return nil, fmt.Errorf("oidc: issuers did not match the issuer returned by provider, expected %q got %q", strings.Join(i, ","), p.Issuer)
	}
